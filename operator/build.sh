...
operator-sdk init --plugins=helm --domain=io --group=ironrain --version=v1alpha1 --kind=IronForge --helm-chart=../../chart/
...
mv --force build/config/samples/ironrain_v1alpha1_ironforge.yaml build/config/crd build/config/manager build/config/default build/config/rbac ./deploy 

mv --force build/Dockerfile build/watches.yaml build/helm-charts ./docker

rm -rf build

cat << EOF >> docker/watches.yaml
  overrideValues:
